...
void main() {
  group('Logger Initialization and Configuration', () {
    ...
    test('Logger should throw an error when reinitialized without reconfiguration', () {
      expect(() => logger.initialize(
        level: LogLevel.info,
        strategies: [ConsoleLogStrategy()],
    ...
    test('Logger should allow reconfiguration with new settings', () {
      expect(() => logger.reconfigure(
        level: LogLevel.debug,
        strategies: [FirebaseCrashlyticsLogStrategy(), FirebaseAnalyticsLogStrategy()],
      ), returnsNormally);
    });
  });
  ...
  group('Logging Functionality', () {
    ...
    test('Logger should log messages at or above the configured log level', () {
      logger.log('This is an info level log', event: LogEvent(eventName: 'INFO_EVENT'));
    ...
    test('Logger should not log messages below the configured log level', () {
      logger.log('This is a debug level log', event: LogEvent(eventName: 'DEBUG_EVENT'));
    ...
    test('Logger should correctly handle error logs', () {
      logger.error('This is an error', event: FirebaseCrashlyticsLogEvent(eventName: 'ERROR_EVENT'));
    ...
    test('Logger should correctly handle fatal logs', () {
      logger.fatal('This is a fatal error', event: FirebaseAnalyticsLogEvent(eventName: 'FATAL_ERROR_EVENT'));
  ...
  group('Event Specific Logging', () {
    ...
    test('Logger should log with ConsoleLogStrategy for a console-specific event', () {
      logger.log('Console specific event log', event: ConsoleLogEvent(eventName: 'CONSOLE_EVENT'));
    ...
    test('Logger should log with FirebaseAnalyticsLogStrategy for an analytics-specific event', () {
      logger.log('Analytics specific event log', event: FirebaseAnalyticsLogEvent(eventName: 'ANALYTICS_EVENT'));
    ...
    test('Logger should handle multiple strategies for a generic event', () {
      logger.log('Generic event affecting multiple strategies', event: LogEvent(eventName: 'GENERIC_EVENT'));
